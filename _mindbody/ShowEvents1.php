	...
	<?php 
	  ...
	  if(isset($_POST['submit'])) {
		  ...
		  if(!isset($hasError)) {
			  ...
			  //echo "got here";
//			  $response = MB_Request ($_MBService, $_MBRequest, $_useDates, $startdateformat, $enddateformat);
			  $response = MB_Request ($_MBService, $_MBRequest, $_useDates, $startdateformat, $startdatetag, $enddateformat, $enddatetag);

			  //$response = MB_Request ($_MBService, $_MBRequest, $_useDates, $startdateformat, $enddateformat, $otherservice, $otherservicevalue);
...
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#e46c0a', endColorstr='#f7c397',GradientType=0 ); /* IE6-8 */
  ...
  <div id="Left60percent">
    <div class="borderRight">
      <h1 class="center paddingTB2pc marginLR10pc underlined">Find Workshops &amp; Seminars
      </h1>

	  <?php //When there are no errors
         if (!(isset($_ResultCount_error))) :?>
      <h3 class="center">Please enter Start and End Date to find Seminars! </h3>
            <?php if($_ResultCount>0) : ?>
            <p class="center">We found  <?=$_ResultCount; ?> Semiars/Workshops.
		   <?php endif; ?>	
      </p>
		 <?php else:?>  
          <h3 class="center"> Sorry there are no Classes in your date range! </h3>
	  ...
	  <div class="marginLR10pc">
        <form action="ShowEvents1.php" method="post" class="formBackground radius8">
          ...
          <br>
          <br>
          <input name="submit" type="submit" class="marginL20 marginB20"  value="Search"/>
        </form>
           ...
           if ((isset($_POST['submit'])) &&($_ResultCount > 0)) :?>
            <h2>Available Seminars</h2>   
              ...
              <div class="MainTxt">
				  ...
				  $classes = toArray($response->$resultTags->$resultTag);
                  ?>
